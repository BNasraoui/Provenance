...
#[test]
fn skills_install_removes_managed_legacy_skill_directories() {
    let dir = tempfile::tempdir().unwrap();
    for skill in [
        "shaping",
        "fork-tournament",
        ...
        let legacy = dir.path().join(".claude/skills").join(skill);
        std::fs::create_dir_all(&legacy).unwrap();
        std::fs::write(
            legacy.join("SKILL.md"),
            "<!-- Installed by provenance 0.1.0; content hash fnv1a64:0123456789abcdef -->\nlegacy\n",
        )
        .unwrap();
    }
    ...
    for skill in [
        ...
        assert!(!dir.path().join(".claude/skills").join(skill).exists());
    }
}

#[test]
fn skills_install_preserves_unmanaged_same_named_legacy_directory() {
    let dir = tempfile::tempdir().unwrap();
    let legacy = dir.path().join(".claude/skills/shaping");
    std::fs::create_dir_all(&legacy).unwrap();
    std::fs::write(legacy.join("SKILL.md"), "user-authored skill\n").unwrap();
...
#[test]
fn skills_install_strips_legacy_agents_section_without_changing_surrounding_content() {
    let dir = tempfile::tempdir().unwrap();
    let agents = dir.path().join("AGENTS.md");
    let before = b"user instructions before\n";
    let legacy =
        b"<!-- BEGIN PROVENANCE SKILLS -->\nmanaged content\n<!-- END PROVENANCE SKILLS -->\n";
    let after = b"user instructions after\n\xff";
    let mut contents = Vec::new();
    contents.extend_from_slice(before);
    contents.extend_from_slice(legacy);
    ...
    assert_eq!(std::fs::read(agents).unwrap(), expected);
}

#[test]
fn skills_install_without_legacy_artifacts_remains_unchanged() {
    let dir = tempfile::tempdir().unwrap();
