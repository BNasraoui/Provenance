...
pub(super) async fn load_scope(
    ...
    let mut loaded = 0;
    for thread in snapshot.threads.clone() {
    ...
    for message in snapshot.messages.clone() {
    ...
    for contribution in snapshot.contributions.clone() {
    ...
    for packet in snapshot.synthesis_packets.clone() {
    ...
    for proposal in snapshot.proposals.clone() {
    ...
    for decision in snapshot.promotion_decisions.clone() {
