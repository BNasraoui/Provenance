...
#[test]
fn skips_a_quoted_marker_but_keeps_a_later_comment_marker() {
    let scan = scan_file(
        Utf8Path::new("fixture.rs"),
        Language::Rust,
        "let fixture = \"@provenance rule: string_only\"; // @provenance rule: real\n\
         fn real_rule() {}",
    );
...
#[test]
fn a_double_quote_character_literal_does_not_hide_a_real_comment_marker() {
    let source = "let quote = '\"'; // @provenance rule: real\nfn real_rule() {}";
    let scan = scan_file(Utf8Path::new("fixture.rs"), Language::Rust, source);
...
#[test]
fn scans_a_real_marker_after_a_multiline_raw_string_closes() {
...
#[test]
fn a_single_quoted_double_quote_does_not_hide_a_python_comment_marker() {
    let source = "print('\"')  # @provenance rule: real\ndef real_rule(): pass";
...
#[test]
fn a_double_quote_inside_a_go_raw_string_does_not_hide_a_real_marker() {
    let source = "value := `\"` // @provenance rule: real\nfunc realRule() {}";
