...
Concurrent writers serialize complete logical mutations with an exclusive generation lock under `.provenance/cache/locks/`. A transaction stages and syncs every replacement before publication, keeps rollback copies, and records a recovery journal before activating any shard. Generation snapshots and exports take the same lock and recover an interrupted publication before reading, so they expose either the old complete generation or the new complete generation. Lock, journal, staging, and rollback files are derived cache artifacts, not state, and must not be committed.
